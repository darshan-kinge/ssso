...
export class SssoClient {
  ...
  async buildAuthorizeUrl(state: string = randomState()): Promise<string> {
  ...
  parseCallback(search: string): { code: string; state: string } {
  ...
  private tokenExchangeBody(code: string): Record<string, string> {
    ...
    if (!secret) {
      throw new Error(
        "No code_verifier or clientSecret — enable PKCE or use server-side exchange"
      );
    }
