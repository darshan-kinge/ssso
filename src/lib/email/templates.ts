...
export function verificationEmailContent(token: string) {
  const { app } = getConfig();
  const link = buildAuthLink("/verify-email", token);
  const hours = getConfig().email.verificationTokenTtlHours;

  const html = layout(
    "Verify your email",
    `
    <p style="color:#a1a1aa;line-height:1.5">Confirm your email for <strong>${app.name}</strong>.</p>
  ...
  const text = `Verify your email for ${app.name}:\n\n${link}\n\nExpires in ${hours} hours.`;
